...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('-c', '--categories', type=int, nargs='?', required=True, help='Number of categories impoted')
        parser.add_argument('-p', '--products', type=int, nargs='?', required=True, help='Number of products imported in each category')

    def handle(self, *args, **options):

        i = 0
        openfoodfact = OpenFoodFacts(options['categories'], options['products'])
        for cat in openfoodfact.categories:
            ...
            i += 1
            self.stdout.write("{}% éffectué ...".format(int(i / options['categories'] * 100)))
